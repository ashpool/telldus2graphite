...
module.exports = function (config) {
	...
	var logformatter = require('./logformatter')(config.format);
		...
		return new RSVP.Promise(function (resolve, reject) {
			if (!sensorInfo) {
				...
				var promises = sensorInfo.data.map(function (data) {
					return logMetric(logformatter.format(sensorInfo, data), (sensorInfo.lastUpdated * 1000) + sensorInfo.timezoneoffset);
				});
